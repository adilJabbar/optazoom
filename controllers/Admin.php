...
class Admin extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->session->userdata('admin_login') == 'yes') {
            ...
            $this->load->view('back/loginadmin',$page_data);
        }
    }
    ...
    function product($para1 = '', $para2 = '', $para3 = '')
    {
        ...
        if ($para1 == 'do_add') {
            ...
            $this->db->where('added_by', json_encode(array('type'=>'admin','id'=> '4' )));
            ...
            $this->db->where('added_by', json_encode(array('type'=>'admin','id'=> '4' )));
            $page_data['all_product'] = $this->db->get('product')->result_array();
            // $page_data['all_product']= $this->db->limit(5);
            // var_dump($page_data);
            // exit();
    ...
    function sales($para1 = '', $para2 = '')
    {
        ...
        elseif ($para1 == 'delivery_payment_set') {
            ...
            $json=json_decode($page_data['get_sale']['shipping_address']);
            $email=$json->email;
            $delivery_status = json_decode($this->db->get_where('sale', array(
                'sale_id' => $para2
            ))->row()->delivery_status,true);
    ...
    $this->load->view('back/includes_top');
    $data['system_name']="Optazoom";
    $this->load->view('back/header',$data);
    $this->load->view('back/admin/edit_customer', $page_data);
    $this->load->view('back/admin/navigation');
    $this->load->view('back/footer');
    $this->load->view('back/script_texts');
    $this->load->view('back/includes_bottom');
    ...
    function contact_message($para1 = "", $para2 = "")
    {
        ...
        if ($para1 == 'delete') {
            ...
            $this->db->select('contact_message.*,  user.username as customerName');
            $this->db->from('contact_message');
            $this->db->join("user","user.user_id = contact_message.customer_id");
            $this->db->join("product","product.product_id = contact_message.product_id");
            $query=$this->db->get();
            ...
            // This Query use to fetch data agains type customer 
            $this->db->select('contact_message.*,  user.email as customerEmail, product.title as productName');
            $this->db->from('contact_message');
            $this->db->where('contact_message.type','customer');
            $this->db->join("user","user.user_id = contact_message.customer_id");
            $this->db->join("product","product.product_id = contact_message.product_id");
            $query=$this->db->get();
            $page_data['customer_query']=$query->result();
            
            // This Query use to fetch data agains type vendor 
            $this->db->select('contact_message.*,  vendor.email as vendorEmail, vendor.company as vendorCompany');
            $this->db->from('contact_message');
            $this->db->where('contact_message.type','vendor');
            $this->db->join("vendor","vendor.vendor_id = contact_message.customer_id");
            $query1=$this->db->get();
            $page_data['vendor_query']=$query1->result();
 ...
 public function queries(){
        $data['queries'] = $this->Crud_model->getrows();
       $this->load->view('back/admin/enquiry', $data);
    ...
    public function answer(){
        ...
        $this->db->where("id",$id);
        $update = $this->db->update("contact_message",$data);
       if($update){
