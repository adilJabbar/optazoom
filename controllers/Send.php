...
class Send extends CI_Controller {
	...
	public function submit(){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->db->insert('vendor_message',$arr);
			$detail = $this->db->select('*')->from('vendor_message')->where('id',$this->db->insert_id())->get()->row();
			$arr['name'] = $detail->name;
			$arr['email'] = $detail->email;
			$arr['subject'] = $detail->subject;
			$arr['created_at'] = $detail->created_at;
			$arr['id'] = $detail->id;
			$arr['new_count_message'] = $this->db->where('read_status',0)->count_all_results('vendor_message');
			$arr['success'] = true;
