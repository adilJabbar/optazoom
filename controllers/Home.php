...
class Home extends CI_Controller
{
    ...
    function vendor_profile($para1 = '', $para2 = '')
    {
        ...
        if ($para1 == 'get_slider') {
            ...
            $view = $this->crud_model->get_view($para1);
            $views =  $view[0]->views;
            $new_views = $views + 1;
    ...
    function profile($para1 = "", $para2 = "", $para3 = "")
    {
        ...
        elseif ($para1 == "query") {
            $page_data['queries'] = $this->db->get_where('contact_message', array('customer_id' => $this->session->userdata('user_id'),'type' => 'customer'))->result_array();

            $this->load->view('front/user/query', $page_data);
        }
         elseif ($para1 == "queryy") {
            $page_data['queriess'] = $this->db->get_where('contact_message', array('customer_id' => $this->session->userdata('user_id'),'type' => 'customer'))->result_array();
    ...
    function all_brands()
    { 
        $page_data['brands'] = $this->Crud_model->get_brand();
        // print_r($page_data['brands']);
    ...
    function product_view($para1 = "", $para2 = "")
    {
        ...
        $page_data['no_of_views'] = $product_data->row()->number_of_view;
                ...
                if ($safe == 'yes') {
                    if ($this->crud_model->get_settings_value('general_settings', 'captcha_status', 'value') == 'ok') {
                        ...
                        if ($this->input->post('password1') == $this->input->post('password2')) {
                            ...
                            $data['password'] = sha1($password);
                            $this->db->insert('vendor', $data);
                            // mail($data['email'],'Optazoom Registration','You have Signed Up for Optazoom successfully');
                            $msg = 'done';
    ...
    function logout()
    {
        if ($this->crud_model->get_settings_value('general_settings', 'fb_login_set') !== 'no') {
            ...
            $id = $this->session->userdata['cart_contents']['bc59e38bc67f18b4ab36cd450302b8c6']['id'];
            $name = $this->session->userdata['cart_contents']['bc59e38bc67f18b4ab36cd450302b8c6']['name'];
    	    $quantity = $this->session->userdata['cart_contents']['bc59e38bc67f18b4ab36cd450302b8c6']['price'];
    	    $shipping = $this->session->userdata['cart_contents']['bc59e38bc67f18b4ab36cd450302b8c6']['shipping'];
    	    $tax = $this->session->userdata['cart_contents']['bc59e38bc67f18b4ab36cd450302b8c6']['tax'];
    	    $subtotal = $this->session->userdata['cart_contents']['bc59e38bc67f18b4ab36cd450302b8c6']['subtotal'];
    	    $link = base_url();  
    	  

         
            // $msg = "Dear Customer, you abandoned your cart with item(s).$name.";
            $id = ($this->session->userdata['user_id']);
                $email['mail'] = $this->Crud_model->getmail($id);
                $mail = $email['mail']['0']->email;
                $message = '
 
  <img class="center" src="https://www.optazoom.com/images/logo.png">
  <h3> Dear Customer, you Abandoned your cart with item(s):' .$name. 'With subtotal : ' .$subtotal.'
  <a href="'.$link.'" target="_blank">Get back to shopping?</a>
';
...
$headers .= 'Content-type: text/html; charset=UTF-8' . "\r\n";
    ...
    if(isset($name)){            
        mail($mail,"Cart Abandoned!",$message,$headers);
    ...
    function invoice($para1 = "", $para2 = "")
    {
        ...
        {
            $data       =   $this->Crud_model->getorders();
            $products   =   $data[0]->product_details;
            $sale_code  =   $data['sales']['0']->sale_code;
            $products   =   json_decode($products,true);
            $products   =   array_pop(array_reverse($products));
            $products   =   (object)$products;
            
           
            $name =$products->name;
             $msg = "'Dear customer, Your ordered '.$name.' against sale code: '.$sale_code.',Thanks for shopping with us'";
                $id = ($this->session->userdata['user_id']);
                $email['mail'] = $this->Crud_model->getmail($id);
                $mail = $email['mail']['0']->email;
    ...
    function review($para1 = "", $para2 = "")
        {
            ...
            $page_data['page_name'] = "shopping_cart/review";
    ...
    public function inquiry(){
        ...
        $data = array(
        'product_id' => $this->input->post('product_id'),
        'question' => $this->input->post('query'),
        ...
        $insert = $this->Crud_model->insertQuestion($data);
       if($insert){
   ...
   public function review_post(){
	...
	function news()
	{
		$this->load->view('front/header/header_1.php');
		
		$this->load->view('front/includes/top/index2.php');
	
		$this->load->view('front/footer/footer_1.php');
	}
	public function mail(){
	  $to_email = 'razaijaz30@gmail.com';
...
mail($to_email,$subject,$message,$headers);
	...
	public function mymale(){
	    $this->load->view('mail');
	}
 public function brands(){
    
     $this->load->view('/front/header/header_1');
     $this->load->view('/front/brands');
     $this->load->view('/front/footer/footer_1');
 }
}
