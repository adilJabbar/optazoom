...
if (!defined('BASEPATH'))
   ...
   require_once(APPPATH.'../ven/autoload.php');
...
//         $sid = 'ACae06c8fb2dcedf6056e8bc08a8436357';
//         $token = 'ebb96be8a84708160892caec41813168';
...
//     }
    ...
    public function index()
    {
        if ($this->session->userdata('vendor_login') == 'yes') {
            ...
            $this->load->view('back/loginvendor',$page_data);
        }
    }
    // public function verify()
    // {
    //     if ($this->session->userdata('vendor_verify') == 'yes') {
    //         $page_data['page_name'] = "dashboard";
    //         // var_dump($this->session->userdata('title'));
    //         // exit();
    //         $this->load->view('back/index', $page_data);
    ...
    public function verifyit()
    {
     ...
     if($code==$random){
         ...
         redirect('vendor/login/verified');
     }
     else
     {   
         $this->session->unset_userdata('random');
         $this->session->unset_userdata('eml');
         $this->session->unset_userdata('pas');
         redirect('vendor');
     }
    ...
    function login($para1 = 'verified')
    {
        ...
        elseif($para1 == 'verified'){
           
                $login_data = $this->db->get_where('vendor', array(
                    'email' => $_POST['email'],
                    'password' => sha1($_POST['password'])
                ));
                //print_r($login_data->num_rows() );die;
            ...
            else
            {
                ...
                if ($login_data->num_rows() > 0) {
                    if($login_data->row()->status == 'approved'){
                        ...
                        $this->session->set_userdata('random', $randomid);
                        $this->session->set_userdata('eml', $this->input->post('email'));
                        $this->session->set_userdata('pas', $this->input->post('password'));
                        $this->load->view('back/verify');
                        ...
                        $client->messages->create(
                        // the number you'd like to send the message to
                        $login_data->row()->phone,
                    [
                        // A Twilio phone number you purchased at twilio.com/console
                        'from' => '+1 865 276 7039',
                        // the body of the text message you'd like to send
                        'body' => 'Your Verification Code is '.$randomid
                    ]
                );
...
if (!empty($product['title'])) {
        ...
        $product_data['image_link'] = $product['image_link'];
        $product_data['UPC'] = $product['UPC'];
        $product_data['EIN'] = $product['EIN'];
        $product_data['GTIN'] = $product['GTIN'];
        $product_data['published'] = $product['published'];
        $product_data['owner'] = $product['owner'];
        $product_data['manufacturer_name'] = $product['manufacturer_name'];
        $product_data['multi_variant_title'] = $product['multi_variant_title'];
        $product_data['multi_variant_fields'] = $product['multi_variant_fields'];
 ...
 /* FUNCTION: Verify vouguepay payment by IPN*/
    ...
    function vendor_images($para1 = "")
    {
       ...
       public function question(){
        ...
        'customer_id' => $this->session->userdata('vendor_id'),
        'type' => 'vendor',
    ...
    function inquiry($para1 = "", $para2 = "")
    {
        ...
        if ($para1 == 'delete') {
            ...
            $page_data['page_name'] = "enquiries";
            $this->db->select('contact_message.*,  vendor.email as vendorEmail');
            $this->db->from('contact_message');
            $this->db->where('contact_message.type','vendor');
            $this->db->join("vendor","vendor.vendor_id = contact_message.customer_id");
            $query=$this->db->get();
            $page_data['contact_messages']=$query->result();
            $this->load->view('back/index', $page_data);
        } elseif ($para1 == 'reply') {
   ...
   function reviews($para1 = "", $para2 = "")
    {
        ...
        if ($para1 == 'delete') {
            ...
            $this->db->select('review.*,  user.username as vendorEmail');
            $this->db->from('review');
            // $this->db->where('contact_message.type','vendor');
            ...
            $this->db->select('review.*,  user.username as vendorEmail');
            $this->db->from('review');
            $this->db->where('review.vendor_id',$this->session->userdata('vendor_id'));
            // print_r($this->session->userdata('vendor_id'));
            // exit;
            $this->db->join("user","user.user_id = review.user_id");
            $query=$this->db->get();
            $page_data['contact_messages']=$query->result();
