...
class Crud_model extends CI_Model
{
    ...
    // Message to vendor
    function message_to_seller_unread_messages($message_thread_id, $user_type)
    {
        $count = 0;
        if ($message_thread_id !== 'all') {
            $msgs = $this->db->get_where('message', array('message_thread_id' => $message_thread_id))->result_array();
        } else if ($message_thread_id == 'all') {
            $msgs = $this->db->get('ticket_message')->result_array();
        }
        foreach ($msgs as $row) {
            $status = json_decode($row['view_status'], true);
    ...
    function message_to_vendor_viewed($message_thread_id, $user_type)
  {
      $msgs = $this->db->get_where('message', array('message_thread_id' => $message_thread_id))->result_array();
      foreach ($msgs as $row) {
          $status = json_decode($row['view_status'], true);
  ...
  function message_to_vendor_unread_messages($message_thread_id, $user_type)
  {
      $count = 0;
      if ($message_thread_id !== 'all')
      {
...
>
    ...
    */
    ...
    public function insertAnswer($data, $id) {
         print_r($id);
         exit;
        $insert = $this->db->insert('queries', $data);
        $this->db->where('id', $id);
    ...
    public function inserviews($data,$para1){
